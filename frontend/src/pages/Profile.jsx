...
function Profile() {
    ...
    const handleConnectGoogleEmail = async () => {
        ...
        try {
            ...
            const response = await api.get('/auth/google/email');
            if (response.data.auth_url) {
                // Redirect to Google OAuth
                window.location.href = response.data.auth_url;
            }
        } catch (error) {
            showToast(error.response?.data?.message || 'Failed to connect Google account', 'error');
            setIsConnecting(false);
        }
    };

    const handleDisconnectEmail = async (emailId) => {
        try {
            await api.delete(`/email-accounts/${emailId}`);
            setEmailAccounts(emailAccounts.filter(email => email.id !== emailId));
            showToast('Email account disconnected successfully', 'success');
        } catch (error) {
            showToast(error.response?.data?.message || 'Failed to disconnect email account', 'error');
        }
    };
                            ...
                            <div className="flex-1 text-center md:text-left">
                                ...
                                <div className="flex justify-center md:justify-start gap-8">
                                    <div className="text-center">
                                        <div className="text-2xl font-bold text-blue-600">{emailAccounts.length}</div>
                        ...
                        {emailAccounts.length === 0 ? (
                            ...
                            <div className="space-y-4">
                                {emailAccounts.map((emailAccount) => (
                                    <div key={emailAccount.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    <p className="font-medium text-gray-900">{emailAccount.email}</p>
                                        ...
                                        <button
                                            onClick={() => handleDisconnectEmail(emailAccount.id)}
                                ...
                                <button
                                    onClick={handleConnectGoogleEmail}
                                    disabled={isConnecting}
                                    className="w-full flex items-center justify-center gap-2 p-4 border-2 border-dashed border-gray-300 rounded-xl text-gray-600 hover:border-blue-400 hover:text-blue-600 transition-colors duration-300 disabled:opacity-50"
                                >
